...
void _1028A::PathGenerator::generatePath(
    const std::initializer_list<Pose> &iwaypoints, const std::string &iid) {
...
void _1028A::PathGenerator::generatePath(
  ...
  for (auto &point : iwaypoints) {
    points.push_back(Waypoint{point.x.convert(okapi::meter),
                              point.y.convert(okapi::meter),
                              point.yaw.convert(okapi::radian)});
  }
  ...
  if (trajectory == nullptr) {
    std::string message =
        "AsyncMotionProfileController: Could not allocate trajectory. ";
  ...
  for (int i = 0; i < candidate.length; i++) {
    poses.emplace_back(InternalDistancePoseIndexed{
        InternalPose{trajectory[i].x, trajectory[i].y, trajectory[i].heading},
        candidate.path_length, candidate.totalLength});
  }
  ...
  std::cout << "Path Generator: Done in " << pros::millis() - time
            << " millis\n";
}
...
std::vector<_1028A::IndexedDistancePosePath> &
_1028A::PathGenerator::getPaths() {

...
void _1028A::odom::Record(void *ptr){
    while (1){
      chassis->getState();
        recorded_points = CurrentPosition.str(inch, "_in", degree, "_deg");
...
double _1028A::odom::get_degrees(std::vector<double> p1, std::vector<double> p2) {
  y2 = p1[0] - p2[0];
  x2 = p1[1] - p2[1];
...
std::vector<double> _1028A::odom::get_intersection(std::vector<double> start, std::vector<double> end, std::vector<double> cur, double radius) {
...
void _1028A::odom::move_to(std::vector<double> pose, double stop_threshold, bool pure_pursuit, std::vector<double> speeds) 
{
    new_y = pose[0];
    new_x = pose[1];
    heading = pose[2];
    ...
    if (pure_pursuit) heading = (abs(pros::Inertial.get_rotation() - heading) < abs(pros::Inertial.get_rotation() - heading2)) ? heading : heading2;
    ...
    while (abs(y_error) > 0.2 || abs(x_error) > 0.2 || abs(imu_error) > 0.5){ 
        ...
        if (motion_average < stop_threshold && timeForMove_To > 100) break;
        ...
        powerPurePursuit = pid::math(powerError, lastPowerError, 1, 0, 1, 127) * speeds[0];
        //double strafe = strafe_PD.get_value(x_error * std::cos(phi) - y_error * std::sin(phi)) * speeds[1];
        turnError = imu_error;
        turnPurePursuit = pid::math(turnError, lastTurnError, 1, 0, 1, 127) * speeds[2];
...
void _1028A::odom::move_to_pure_pursuit(std::vector<std::vector<double>> points, std::vector<double> final_point, std::vector<double> speeds)
{
...
std::vector<std::vector<double>> _1028A::odom::smoothing (std::vector<std::vector<double>> path, double weight_data, double weight_smooth, double tolerance) {
...
std::vector<std::vector<double>> _1028A::odom::auto_smooth (std::vector<std::vector<double>> path, double max_angle) {
  ...
  while (current_max >= max_angle || first_loop) { // && (counter <= 15) {
    ...
    for (int i = 1; i < new_path.size() - 2; i++) {
      double angle1 = atan2(new_path[i][1] - new_path[i-1][1], new_path[i][0] - new_path[i-1][0]) * 180 / M_PI;
      if (angle1 < 0) angle1 += 360;

      double angle2 = atan2(new_path[i+1][1] - new_path[i][1], new_path[i+1][0] - new_path[i][0]) * 180 / M_PI;

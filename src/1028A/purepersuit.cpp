...
_1028A::PurePursuit::PurePursuit( const std::vector<IndexedDistancePosePath> &ipaths, const okapi::QLength &ilookaheadDistance ): paths(ipaths){
    std::cout << "\nPure Pursuit: Initializ";
    lookaheadDistance = ilookaheadDistance.convert(okapi::meter);
    std::cout << "ed\n";
}

_1028A::PurePursuitTriangle _1028A::PurePursuit::run( const okapi::OdomState &ipose, const std::string &iid ){
    okapi::OdomState pose = std::move(ipose);
    return run( Pose{ pose.x, pose.y, pose.theta }, iid );
}

_1028A::PurePursuitTriangle _1028A::PurePursuit::run( const Pose &ipose, const std::string &iid ){
...
void _1028A::PurePursuit::updateChassis( const double &reqVelocity, const PurePursuitTriangle &itriangle, const std::shared_ptr<okapi::OdomChassisController> &controller ){
    std::cout << "Pure Pursuit: Updating Chassis\n";
    PurePursuitTriangle triangle = itriangle;
    double curvature_scales = triangle.localGoalPose.yaw * (controller->getChassisScales().wheelTrack.convert(okapi::meter))/2;
...
double _1028A::PurePursuit::findDistanceBetweenPoses( const InternalPose &P1, const InternalPose &P2 ){
